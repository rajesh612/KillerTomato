=========================================
Oracle Containers for J2EE 10g (10.1.3.5)
=========================================
...
Application Server Control console to configure the server and deploy 
applications.

A small set of additional features have been added to the OC4J 10g (10.1.3.5) 
release.  

    * Peek: a graphical utility for interacting with the OC4J classloader 
    * New administration commands for admin_client.jar/Ant tasks:
        + Application level shared-library commands, import-shared-library, 
          remove-inherited 
        + Website creation 
        + Web module unbind, unbindall
        + List Applications
        + List Web bindings
        + List DataSources
        + List DataSource Connection Pools
    * Parent application redeployment without affecting child applications
    * Redeployment failure handling with a recovery option
    * Deployment using exploded-directory using admin_client.jar 
    * Oracle WS libraries migrated to named shared libraries (10.1.3.4)


Prerequisites
--------------

In order to use the OC4J zip distribution, a Java Platform Standard
Edition (Java SE) Development Kit version 6.0, 5.0 (or 1.4.2) must be installed 
on the host machine.
...
Extract the oc4j_extended.zip file using any archive utility that handles zip
files, including the jar utility provided with Java SE.
...
For example, if oc4j_extended.zip was extracted to /java/oc4j-10135 and the
Java SE SDK was installed in /java/j2se15 then JAVA_HOME and ORACLE_HOME would 
be set as:

    JAVA_HOME=/java/j2se15
    ORACLE_HOME=/java/oc4j-10135


First Time Initialization
-------------------------

There are no specific tasks that need to be performed in order to use the new OC4J
instance.  The new OC4J instance can be used immediately after the unzip
operation has completed.

** Note: on the first use of OC4J, you are prompted to enter a password for
...
Change to the <install-dir>/bin directory, and issue the following command:
    ...
    Starting OC4J from /java/oc4j-10135/j2ee/home ...

    ... <application deployment messages >

    08/11/16 13:57:17 Set OC4J administrator's password (password text will not
    be displayed as it is entered)
    Enter password: ******
    Confirm password: ********
    The password for OC4J administrator "oc4jadmin" has been set.
    08/11/16 13:57:17  The OC4J administrator "oc4jadmin" account is activated.
    08/11/16 13:57:22 The OC4J administrator "oc4jadmin" account is activated.
    08/11/16 13:57:31 Oracle Containers for J2EE 10g (10.1.3.5.0) initialized
...
Start OC4J using the command script as follows:
    ...
    Starting OC4J from /java/oc4j-10135/j2ee/home ...
    08/11/16 14:00:38 Oracle Containers for J2EE 10g (10.1.3.5.0)  initialized
...
Directly start OC4J server by changing to the <install_dir>\j2ee\home directory,
referred to as J2EE_HOME, and issuing one of the following commands:
...
ready to accept requests. For example:
    ...
    Oracle Containers for J2EE 10g (10.1.3.5.0) initialized
...
Stop OC4J using one of these methods:
    ...
    + Use the process termination command (for example "kill" on Unix/Linux) for 
      the operating system in use. OC4J indicates on the console window that 
      it is shutting down when it receives a shutdown signal.
...
Peek is a graphical utility that is used to interact with the OC4J
classloading system. Peek exposes the structure of the current loader tree and 
allows you to:

    * Drill down into the current loader tree
    * View and traverse the loader hierarchy
    * View the set of loaded classes
    * View the configuration of the loaders

Peek also includes in-built classloader queries that can be executed in order to
assist with the diagnosis of classloading related problems.

The Peek classloading utility can be accessed using the URL:
    
    http://<oc4j_host>:8888/peek
    
To logon to Peek, use the "oc4jadmin" username and the password you set during 
the first initialization of OC4J.
...
means OC4J must be started before using the utility. OC4J cannot be started
...
Use the -help option to see all of the available admin_client.jar commands.
...
To deploy an application from an exploded-directory: 
...
To view the list of Web bindings:
    
    >java -jar admin_client.jar deployer:oc4j:localhost oc4jadmin <admin_pwd>    
     -listWebBindings
     
To add an import shared library element to a deployed application:

    >java -jar admin_client.jar deployer:oc4j:localhost oc4jadmin <admin_pwd>
     -addImportSharedLibrary -appName <app> -name <name> 
...
To delete an imported shared library element from a deployed application:
...
To add a remove inherited shared library element to a deployed application:
...
To delete a remove inherited shared library element from a deployed application:
